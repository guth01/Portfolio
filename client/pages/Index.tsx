...
const projects = [
  {
    ...
    highlights: [
      "Architected an Agentic RAG pipeline combining ephemeral ChromaDB stores, query translation, and Cohere vector reranking, backed by a semantic Sufficiency Judge triggering Tavily search fallbacks.",
      "Built a stateful LangGraph backend to process Cloudinary-hosted PDFs, generating persistent interactive flashcards and automated PDF revision sheets grounded in the user's uploaded context.",
      "Engineered a multi-key LLM routing system (dual Gemini keys, Groq) to bypass rate limits during intensive reasoning, and integrated human-in-the-loop interrupts to securely insert study schedules via Google Calendar API.",
    ],
  ...
  },
  {
    ...
    tags: ["React", "Node.js", "Express.js", "Supabase", "Redis", "Docker", "Render"],
    highlights: [
      "Engineered a Node.js/Express and PostgreSQL backend for a gamification engine — real-time XP/leaderboard updates, timezone-aware cron jobs for streaks and quests, plus a JSON-rule-driven badge engine.",
      "Built an interactive React/TypeScript Kanban app with drag-and-drop tasks, an integrated Pomodoro timer and lo-fi player, and Google OAuth 2.0 with CSRF-protected state handling.",
      "Integrated Google Gemini for weekly AI progress reports with graceful fallback on failure, and built a hybrid Redis/PostgreSQL leaderboard with O(log N) real-time rankings.",
    ],
  ...
  },
  {
    ...
    highlights: [
      "Built and published an open-source Node.js MCP server to NPM and the official MCP Registry, giving LLMs operational control over live events via 20+ custom tools.",
      "Implemented dual transport modes (stdio and HTTP/SSE), enabling zero-config local integration with any standard MCP client (e.g., Claude Desktop, Cursor) as well as remote cloud deployment.",
      "Integrated real-time external data sources including Google Sheets, Google Maps, and OpenWeather APIs, among other tools, to drive autonomous AI decision-making.",
    ],
...
const experience = [
  {
    ...
    highlights: [
      "Worked alongside Frontend (Angular), Backend (Java Spring Boot), and Android (Kotlin) engineering teams under mentorship, implementing guided tasks while gaining practical experience with enterprise development workflows.",
      "Designed and developed a microservices-based Restaurant POS prototype using Spring Boot, implementing JWT authentication, an API Gateway, and OpenFeign inter-service communication.",
      "Integrated WebSockets for real-time bi-directional order updates, RabbitMQ for asynchronous event-driven messaging, Razorpay API for secure payments, and Docker for containerized deployment.",
    ],
  },
];
